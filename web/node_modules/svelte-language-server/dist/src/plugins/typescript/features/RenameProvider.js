...
class RenameProviderImpl {
    ...
    async rename(document, position, newName) {
        ...
        let convertedRenameLocations = await this.mapAndFilterRenameLocations(renameLocations, docs);
        // eslint-disable-next-line max-len
        const additionalRenameForPropRenameInsideComponentWithProp = await this.getAdditionLocationsForRenameOfPropInsideComponentWithProp(document, tsDoc, fragment, position, convertedRenameLocations, docs, lang);
    ...
    getRenameInfo(lang, tsDoc, offset) {
        var _a;
        ...
        if (!renameInfo.canRename ||
            renameInfo.kind === typescript_1.default.ScriptElementKind.jsxAttribute || ((_a = renameInfo.fullDisplayName) === null || _a === void 0 ? void 0 : _a.includes('JSX.IntrinsicElements'))) {
            return null;
        }
        return renameInfo;
    }
     ...
     */
    async getAdditionLocationsForRenameOfPropInsideComponentWithProp(document, tsDoc, fragment, position, convertedRenameLocations, fragments, lang) {
        ...
        const regex = new RegExp(`export\\s+let\\s+${this.getVariableAtPosition(tsDoc, fragment, lang, position)}($|\\s|;|:)`);
     ...
     */
    async getAdditionalLocationsForRenameOfPropInsideOtherComponent(convertedRenameLocations, fragments, lang) {
        ...
        const doc = fragments.get(updatePropLocation.fileName);
    ...
    isInSvelte2TsxPropLine(fragment, loc) {
        const pos = documents_1.positionAt(loc.textSpan.start, fragment.text);
        const textInLine = fragment.text.substring(documents_1.offsetAt({ ...pos, character: 0 }, fragment.text), loc.textSpan.start);
     ...
     */
    async mapAndFilterRenameLocations(renameLocations, fragments) {
    ...
    filterWrongRenameLocations(mappedLocations) {
        return mappedLocations.filter((loc) => {
            ...
            return (notPrecededBy('__sveltets_instanceOf(') && notPrecededBy('__sveltets_ensureType('));
