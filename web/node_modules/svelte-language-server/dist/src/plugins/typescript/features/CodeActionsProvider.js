...
class CodeActionsProviderImpl {
    ...
    async organizeImports(document) {
        ...
        const documentChanges = await Promise.all(changes.map(async (change) => {
            ...
            return vscode_languageserver_1.TextDocumentEdit.create(vscode_languageserver_1.VersionedTextDocumentIdentifier.create(document.url, null), change.textChanges.map((edit) => {
    ...
    async applyQuickfix(document, range, context) {
        ...
        const codeFixes = lang.getCodeFixesAtPosition(tsDoc.filePath, start, end, errorCodes, {}, {});
        ...
        return await Promise.all(codeFixes.map(async (fix) => {
            const documentChanges = await Promise.all(fix.changes.map(async (change) => {
                ...
                return vscode_languageserver_1.TextDocumentEdit.create(vscode_languageserver_1.VersionedTextDocumentIdentifier.create(utils_1.pathToUrl(change.fileName), null), change.textChanges.map((edit) => {
                    if (fix.fixName === 'import' &&
                        doc instanceof DocumentSnapshot_1.SvelteSnapshotFragment) {
                        return this.completionProvider.codeActionChangeToTextEdit(document, doc, edit, true);
                    }
                    return vscode_languageserver_1.TextEdit.replace(documents_1.mapRangeToOriginal(doc, utils_2.convertRange(doc, edit.span)), edit.newText);
                }));
            }));
    ...
    async getApplicableRefactors(document, range) {
        ...
        const applicableRefactors = lang.getApplicableRefactors(document.getFilePath() || '', textRange, undefined);
        return (this.applicableRefactorsToCodeActions(applicableRefactors, document, range, textRange)
            ...
            .filter((refactor) => {
            var _a, _b;
            return ((_a = refactor.command) === null || _a === void 0 ? void 0 : _a.command.includes('function_scope')) || ((_b = refactor.command) === null || _b === void 0 ? void 0 : _b.command.includes('constant_scope'));
        })
    ...
    applicableRefactorsToCodeActions(applicableRefactors, document, originalRange, textRange) {
        return utils_3.flatten(applicableRefactors.map((applicableRefactor) => {
    ...
    async executeCommand(document, command, args) {
        ...
        const edits = lang.getEditsForRefactor(path, {}, textRange, refactorName, command, undefined);
        if (!edits || edits.edits.length === 0) {
            return null;
        }
        const documentChanges = edits === null || edits === void 0 ? void 0 : edits.edits.map((edit) => vscode_languageserver_1.TextDocumentEdit.create(vscode_languageserver_1.VersionedTextDocumentIdentifier.create(document.uri, null), edit.textChanges.map((edit) => {
            let range = documents_1.mapRangeToOriginal(fragment, utils_2.convertRange(fragment, edit.span));
