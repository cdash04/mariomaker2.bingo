...
class SvelteFragmentMapper {
    ...
    static async createSourceMapper(processed, originalDoc) {
        return processed.reduce(async (parent, processedSingle) => (processedSingle === null || processedSingle === void 0 ? void 0 : processedSingle.map) ? new documents_1.SourceMapDocumentMapper(await new source_map_1.SourceMapConsumer(processedSingle.map.toString()), originalDoc.uri, await parent)
...
async function transpile(document, preprocessors = []) {
    ...
    const transpiled = (await svelte.preprocess(document.getText(), wrappedPreprocessors, {
