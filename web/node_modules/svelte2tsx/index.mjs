...
import ts__default, { createSourceFile, ScriptTarget, ScriptKind, isImportDeclaration, isInterfaceDeclaration, isVariableStatement, NodeFlags, isVariableDeclaration, isIdentifier as isIdentifier$1, isFunctionDeclaration, isClassDeclaration, isBlock, isArrowFunction, isExportDeclaration, isNamedExports, isCallExpression, isBindingElement, isImportClause, isLabeledStatement, forEachChild, isBinaryExpression, SyntaxKind, isPrefixUnaryExpression, isPostfixUnaryExpression, isParameter, isPropertyAccessExpression, isPropertyAssignment, isObjectBindingPattern, isArrayBindingPattern, isObjectLiteralExpression, getJSDocType } from 'typescript';
...
function handleActionDirective(htmlx, str, attr, parent) {
    str.overwrite(attr.start, attr.start + 'use:'.length, `{...__sveltets_ensureAction(__sveltets_mapElementTag('${parent.name}'),`);
    if (!attr.expression) {
 ...
 */
function handleAnimateDirective(htmlx, str, attr) {
 ...
 * {@debug ...}   --->   {...}
 */
function handleDebug(htmlx, str, debugBlock) {
 ...
 */
function handleTransitionDirective(htmlx, str, attr) {
...
function getLeadingDoc(node) {
    ...
    const comment = (_a = ts__default
        .getLeadingCommentRanges(nodeText, 0)) === null || _a === void 0 ? void 0 : _a.find((c) => c.kind === ts__default.SyntaxKind.MultiLineCommentTrivia);
    if (comment) {
...
function handleStore(node, parent, str) {
    ...
    if (parent.type == 'AssignmentExpression' &&
        ...
        str.overwrite(parent.start, str.original.indexOf('=', node.end) + 1, `${storename}.set( __sveltets_store_get(${storename}) ${operator}`);
        str.appendLeft(parent.end, ')');
        return;
    }
    ...
    if (parent.type == 'UpdateExpression') {
        ...
        if (simpleOperator) {
            const storename = node.name.slice(1); // drop the $
            str.overwrite(parent.start, parent.end, `${storename}.set( __sveltets_store_get(${storename}) ${simpleOperator} 1)`);
        }
        else {
...
function processInstanceScriptContent(str, script, events) {
    const htmlx = str.original;
    ...
    const handleStore = (ident, parent) => {
        ...
        if (isBinaryExpression(parent) &&
            ...
            str.overwrite(parent.getStart() + astOffset, str.original.indexOf('=', ident.end + astOffset) + 1, `${storename}.set( __sveltets_store_get(${storename}) ${operator}`);
            str.appendLeft(parent.end + astOffset, ')');
            return;
        }
        // handle $store++, $store--, ++$store, --$store
        if ((isPrefixUnaryExpression(parent) || isPostfixUnaryExpression(parent)) &&
            ...
            if (simpleOperator) {
                const storename = ident.getText().slice(1); // drop the $
                str.overwrite(parent.getStart() + astOffset, parent.end + astOffset, `${storename}.set( __sveltets_store_get(${storename}) ${simpleOperator} 1)`);
                return;
            }
            else {
...
function processSvelteTemplate(str) {
...
function svelte2tsx(svelte, options) {
    ...
    let { moduleScriptTag, scriptTag, slots, uses$$props, uses$$slots, uses$$restProps, events, componentDocumentation, } = processSvelteTemplate(str);
